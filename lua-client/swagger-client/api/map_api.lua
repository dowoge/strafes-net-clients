...
local function new_map_api(host, basePath, schemes)
	local schemes_map = {}
...
function map_api:map_get(game, page)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		path = string.format("%s/map?game=%s&page=%s",
			self.basePath, http_util.encodeURIComponent(game), http_util.encodeURIComponent(page));
	})
...
function map_api:map_map_id_get(map_id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		path = string.format("%s/map/%s",
			self.basePath, map_id);
	})
